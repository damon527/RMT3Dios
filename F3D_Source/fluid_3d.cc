...
#endif
        std::vector<double> acc_local;
        mgmt->hit_model.reset_local_acc(acc_local);
        for (int kk =0; kk < so; kk++) {
            double zz = lz0[kk];
            for (int jj =0; jj < sn; jj++) {
                double yy = ly0[jj];
                for (int ii = 0; ii < sm; ii++) {
                    ...
                    field &node = u0[eid];
                    mgmt->hit_model.accumulate_projection(xx, yy, zz, node.vel[0], node.vel[1], node.vel[2], acc_local);
                }
            }
        }

        std::vector<double> acc_global(acc_local.size(), 0.0);
        MPI_Allreduce(acc_local.data(), acc_global.data(), acc_local.size(), MPI_DOUBLE, MPI_SUM, grid->cart);
        double n_cells_global = static_cast<double>(m)*static_cast<double>(n)*static_cast<double>(o);
        mgmt->hit_model.update_from_global_acc(acc_global, n_cells_global);

        mgmt->hit_energy_total = mgmt->hit_model.energy_total;
        mgmt->hit_energy_low = mgmt->hit_model.energy_low;
        mgmt->hit_energy_high = mgmt->hit_model.energy_high;
        mgmt->hit_var[0] = mgmt->hit_model.var_comp[0];
        mgmt->hit_var[1] = mgmt->hit_model.var_comp[1];
        mgmt->hit_var[2] = mgmt->hit_model.var_comp[2];
        mgmt->hit_target_error = mgmt->hit_model.target_error;

        double_int gmx_ext, gmn_ext;
        mgmt->hit_div_l2 = div_u(gmx_ext, gmn_ext);
        mgmt->hit_div_phys_l2 = mgmt->hit_div_l2;
        mgmt->hit_alpha = 1.0;
