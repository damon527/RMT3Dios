...
class sim_manager {
    ...
    /** HIT diagnostics (updated in timestep loop) */
    double hit_energy_total;
    double hit_energy_low;
    double hit_energy_high;
    double hit_var[3];
    double hit_div_l2;
    double hit_div_phys_l2;
    double hit_alpha;
    double hit_forcing_l2;
    long hit_fft_forward_calls;
    long hit_fft_inverse_calls;
    std::vector<double> hit_shell_E;
    double hit_target_error;
    ...
    weight_fac(spars->weight_fac), vmax(1), hit_use_fft(false),
    ...
    hit_energy_total(0), hit_energy_low(0), hit_energy_high(0),
    hit_var{0,0,0}, hit_div_l2(0), hit_div_phys_l2(0), hit_alpha(1), hit_forcing_l2(0),
    hit_fft_forward_calls(0), hit_fft_inverse_calls(0), hit_target_error(0),
  ...
  /** adds HIT low-k forcing contributions */
	void isotropic_turbulence_forcing(double x,double y,double z,double t,
		double dt, double &fx,double &fy,double &fz) const;
	/** setup HIT model from cfg/profile */
	void setup_hit_model();
	void update_hit_state_from_velocity(const std::vector<double> &ux, const std::vector<double> &uy, const std::vector<double> &uz, double dt);
