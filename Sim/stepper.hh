...
void stepper::collect_macro_data(p_class &pr){
    ...
    if(dump_code & 32) {
        ...
        if(pr.rank==0) {
            ...
            if(!file_exist || curr_tot_step == init_step) {
                ...
                fprintf(fh, "%10s %6s %6s "
                            ...
                            "hit_k", "hit_low", "hit_high", "hit_var_x", "hit_var_y", "hit_var_z", "hit_div", "hit_div_p", "hit_alpha", "hit_f_l2", "hit_err");
                fclose(fh);
            }
        }
        ...
        if(pr.rank==0) {
            ...
            fprintf(fh, "%.*e %.*e %.*e %.*e %.*e %.*e %.*e %.*e %.*e %.*e %.*e", digs, pr.mgmt->hit_energy_total, digs, pr.mgmt->hit_energy_low, digs, pr.mgmt->hit_energy_high,
                    digs, pr.mgmt->hit_var[0], digs, pr.mgmt->hit_var[1], digs, pr.mgmt->hit_var[2], digs, pr.mgmt->hit_div_l2, digs, pr.mgmt->hit_div_phys_l2,
                    digs, pr.mgmt->hit_alpha, digs, pr.mgmt->hit_forcing_l2, digs, pr.mgmt->hit_target_error);
            fprintf(fh, "\n");
            fclose(fh);

            // Stage-2 FFTW HIT verification files.
            sprintf(fname_buf, "%s/hit_energy.dat", dir);
            bool exists = false;
            fh = fopen(fname_buf, "r");
            if(fh!=NULL){ fclose(fh); exists = true; }
            fh = p_safe_fopen(fname_buf, exists?"a":"w");
            if(!exists || curr_tot_step==init_step) fprintf(fh, "# t step E_total E_low E_high alpha target_err var_x var_y var_z forcing_l2\n");
            fprintf(fh, "%.*e %d %.*e %.*e %.*e %.*e %.*e %.*e %.*e %.*e %.*e\n", digs, pr.time, curr_tot_step,
                    digs, pr.mgmt->hit_energy_total, digs, pr.mgmt->hit_energy_low, digs, pr.mgmt->hit_energy_high,
                    digs, pr.mgmt->hit_alpha, digs, pr.mgmt->hit_target_error,
                    digs, pr.mgmt->hit_var[0], digs, pr.mgmt->hit_var[1], digs, pr.mgmt->hit_var[2], digs, pr.mgmt->hit_forcing_l2);
            fclose(fh);

            sprintf(fname_buf, "%s/hit_divergence.dat", dir);
            exists = false;
            fh = fopen(fname_buf, "r");
            if(fh!=NULL){ fclose(fh); exists = true; }
            fh = p_safe_fopen(fname_buf, exists?"a":"w");
            if(!exists || curr_tot_step==init_step) fprintf(fh, "# t step div_phys_l2 div_spec_l2 fft_forward_calls fft_inverse_calls\n");
            fprintf(fh, "%.*e %d %.*e %.*e %ld %ld\n", digs, pr.time, curr_tot_step,
                    digs, pr.mgmt->hit_div_phys_l2, digs, pr.mgmt->hit_div_l2,
                    pr.mgmt->hit_fft_forward_calls, pr.mgmt->hit_fft_inverse_calls);
            fclose(fh);

            if(!pr.mgmt->hit_shell_E.empty()) {
                sprintf(fname_buf, "%s/hit_spectrum.dat", dir);
                exists = false;
                fh = fopen(fname_buf, "r");
                if(fh!=NULL){ fclose(fh); exists = true; }
                fh = p_safe_fopen(fname_buf, exists?"a":"w");
                if(!exists || curr_tot_step==init_step) fprintf(fh, "# t step kbin E_k\n");
                for(size_t kb=0; kb<pr.mgmt->hit_shell_E.size(); kb++) {
                    fprintf(fh, "%.*e %d %zu %.*e\n", digs, pr.time, curr_tot_step, kb, digs, pr.mgmt->hit_shell_E[kb]);
                }
                fclose(fh);
            }
        }
...
void stepper::run_diagnostics(p_class &pr){
    ...
    if(dump_code & 16) {
        ...
        if(pr.rank==0) {
            ...
            fprintf(fh, "%+10.4g %+10.4g %+10.4g %+10.8g ",  centx, centy, centz, vol);
            fprintf(fh, "%+10.4e %+10.4e %+10.4e %+10.4e %+10.4e %+10.4e %+10.4e %+10.4e %+10.4e %+10.4e %+10.4e\n",
                    pr.mgmt->hit_energy_total, pr.mgmt->hit_energy_low, pr.mgmt->hit_energy_high,
                    pr.mgmt->hit_var[0], pr.mgmt->hit_var[1], pr.mgmt->hit_var[2], pr.mgmt->hit_div_l2, pr.mgmt->hit_div_phys_l2,
                    pr.mgmt->hit_alpha, pr.mgmt->hit_forcing_l2, pr.mgmt->hit_target_error);
            fclose(fh);
        }
	/*
