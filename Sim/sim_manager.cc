...
void sim_manager::update_hit_state_from_velocity(const std::vector<double> &ux, const std::vector<double> &uy, const std::vector<double> &uz, double dt){
    if(hit_use_fft && hit_fft.enabled) {
        ...
        hit_var[0] = hit_fft.var_comp[0];
        hit_var[1] = hit_fft.var_comp[1];
        hit_var[2] = hit_fft.var_comp[2];
        hit_div_l2 = hit_fft.div_l2;
        hit_div_phys_l2 = hit_fft.div_phys_l2_last;
        hit_alpha = hit_fft.alpha_last;
        hit_forcing_l2 = hit_fft.forcing_l2_last;
        hit_fft_forward_calls = hit_fft.fft_forward_calls;
        hit_fft_inverse_calls = hit_fft.fft_inverse_force_calls;
        hit_shell_E = hit_fft.shell_E;
        hit_target_error = hit_fft.target_error;
    }
}
